    ...
    <div className="group/section flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden py-1 pr-1">
        <div>
          {projects.map((p) => (
            (() => {
              const projectActive = !!activeSessionId && p.sessionIds.includes(activeSessionId)
              ...
              return (
                ...
                >
                  ...
                  <ul className="relative px-1.5">
                    {p.sessionIds.map((sid) => {
                      const s = sessions[sid]
                      if (!s) return null
                      const first = p.sessionIds[0] === sid
                      const last = p.sessionIds[p.sessionIds.length - 1] === sid
                      ...
                      return (
                        <li
                          ...
                          className={cn(
                            'group relative flex items-center gap-2 rounded-md px-2.5 py-1.5 text-sm',
                        ...
                        >
                          {projectActive && !first && (
                            <span
                              className="project-session-rail-top pointer-events-none absolute left-[13.5px] top-0 h-1/2 w-px bg-muted-foreground/35"
                              aria-hidden="true"
                            />
                          )}
                          {projectActive && !last && (
                            <span
                              className="project-session-rail-bottom pointer-events-none absolute bottom-0 left-[13.5px] h-1/2 w-px bg-muted-foreground/35"
                              aria-hidden="true"
                            />
                          )}
                          <ShellStatusMark active={active} running={running} />
